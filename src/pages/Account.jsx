      ...
      <div className="account-container">
        <div className="account-layout">
          ...
          <div className="account-content">
            ...
            <div className={`account-panel${tab === 'My Orders' ? ' active' : ''}`} id="ordersPanel"><h2 className="section-title">My Orders</h2><div id="ordersContainer">{orders.length === 0 ? <p>No orders yet</p> : orders.map((order) => <div className="order-card active" key={order?.id}><div className="order-flex"><div className="order-image"><img src={order?.products?.[0]?.image || FALLBACK_IMAGE} className="order-main-img" /></div><div className="order-details"><div className="order-id"><strong>Order ID:</strong> {order?.id}</div><div className="order-date">{new Date(order?.date).toLocaleString()}</div><div className="order-meta"><span><strong>{order?.products?.map((item) => item?.name || 'Unnamed Product').join(', ')}</strong></span><span>{formatPrice(order?.total || 0)}</span><span>{order?.status}</span></div><div className="order-meta"><span>{order?.address?.address1}{order?.address?.address2 ? `, ${order.address.address2}` : ''}</span><span>{order?.address?.city}, {order?.address?.state} - {order?.address?.pincode}</span><span>{order?.address?.phone}</span></div><div className="order-status-badge">{order?.status}</div></div></div></div>)}</div></div>
            <div className={`account-panel${tab === 'Wishlist' ? ' active' : ''}`}><h2 className="section-title">Wishlist</h2><div id="wishlistGrid">{wishlist.length === 0 ? <p>Your wishlist is empty</p> : wishlist.map((item) => <div className="order-card active" key={item?.id}><div className="order-flex"><div className="order-image"><img src={item?.image || FALLBACK_IMAGE} className="order-main-img" /></div><div className="order-details"><div className="order-meta"><span><strong>{item?.name || 'Unnamed Product'}</strong></span><span>{formatPrice(item?.price || 0)}</span><button className="delete-btn" onClick={() => removeItem(item?.id)}>Remove</button></div></div></div></div>)}</div></div>
