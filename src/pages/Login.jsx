  ...
  return (
    <>
      <style>{`
        ...
        .nt-auth-logo img{width:130px;max-width:100%;height:auto;filter:drop-shadow(0 10px 30px rgba(212,175,55,0.4));}
      `}</style>
      <div className="auth-clean">
        <div className="auth-card">
          <div className="nt-auth-logo"><img src={logoPath} alt="Naarithread Logo" /></div>
          <h2>Sign In</h2>
          <p className="auth-sub">Welcome back to Naarithread</p>
          <form onSubmit={(event) => {
          ...
          <p style={{ marginTop: 20, color: '#aaa', fontSize: 14 }}>Don’t have an account? <Link to="/register" style={{ color: '#d4af37', textDecoration: 'none' }}>Create Account</Link></p>
        </div>
      </div>
    </>
  )
